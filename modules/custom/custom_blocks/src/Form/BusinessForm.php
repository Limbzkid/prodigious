<?php
  /**
    * @file
    * Contains \Drupal\custom_blocks\Form\BusinessForm.
    */
  ...
  class BusinessForm extends FormBase {
    /**
    * {@inheritdoc}
    */
    ...
    */
    ...
    public function businessCallback(array &$form, FormStateInterface $form_state) {
      ...
      if($name_err) {
        $css = ['display' => 'block'];
        $ajax_response->addCommand(new CssCommand('.business .invalid-feedback', $css));
        $ajax_response->addCommand(new HtmlCommand('.business .nameErr', $err_txt));
      } else {
        $ajax_response->addCommand(new HtmlCommand('.business .nameErr', ''));
      }
      ...
      if($mail_err) {
        $css = ['display' => 'block'];
        $ajax_response->addCommand(new CssCommand('.business .invalid-feedback', $css));
        $ajax_response->addCommand(new HtmlCommand('.business .mailErr', $err_txt));
      } else {
        $ajax_response->addCommand(new HtmlCommand('.business .mailErr', ''));
      }
      ...
      if($loc_err) {
        $css = ['display' => 'block'];
        $ajax_response->addCommand(new CssCommand('.business .invalid-feedback', $css));
        $ajax_response->addCommand(new HtmlCommand('.business .locErr', $err_txt));
      } else {
        $ajax_response->addCommand(new HtmlCommand('.business .locErr', ''));
      }
      ...
      if($msg_err) {
        $css = ['display' => 'block'];
        $ajax_response->addCommand(new CssCommand('.business .invalid-feedback', $css));
        $ajax_response->addCommand(new HtmlCommand('.business .msgErr', $err_txt));
      } else {
        $ajax_response->addCommand(new HtmlCommand('.business .msgErr', ''));
      }
      ...
      if(!$error) {
        ...
        $mail_body  = '<table  cellpadding="0" cellspacing="0"  style="background: #f3f3f3; padding: 20px; width: 700px; border: 0; text-align: left; font-family:Arial; font-size:14px; color:#000;">
                          ...
                          <tr><td style="padding-top: 15px">Type</td><td style="padding-top: 15px">:</td><td style="padding-top: 15px">Business Enquiry</td></tr>
                          <tr>
        ...
        $mail->Subject = 'Prodigious Website Business Enquiry';
        $mail->Body = $mail_body;
        $mail->isSMTP();
        $mail->isHTML(TRUE);
        ...
        $ajax_response->addCommand(new InvokeCommand('.business #name', 'val',['']));
        $ajax_response->addCommand(new InvokeCommand('.business #email', 'val',['']));
        //$ajax_response->addCommand(new InvokeCommand('.business #inputlocation','val',['']));
        $ajax_response->addCommand(new InvokeCommand('.business #location','val',['']));
        $ajax_response->addCommand(new InvokeCommand('.business #message', 'val',['']));
        $ajax_response->addCommand(new HtmlCommand('.business .feedback', $thank_you));
      } else {
        $ajax_response->addCommand(new HtmlCommand('.business .feedback', ''));
      }
